...
pval_diff <- read.csv("THCAdata_filter.csv",header=T,check.names=FALSE) ## Differentially expressed dataset
...
TF_miRNA_association <- read.csv("TF_miRNA_relation.csv",header=T,check.names=FALSE)
...
TFsexpress <- pval_diff[,which(colnames(pval_diff)%in%colnames(pval_diff[,2:93]))]  ## TFs expression dataset
...
exp_new <- full_data[1:508,]   ## case group tf/gene
ctl_new <- full_data[509:567,]  ## control group tf/gene
exp_new_miRNA <- full_data_miRNA[1:508,]   ## case group miRNA
ctl_new_miRNA <- full_data_miRNA[509:567,]   ## control group miRNA
exp_new_miRNA_gene_es <- full_data_miRNA_gene_es[1:508,]
ctl_new_miRNA_gene_es <- full_data_miRNA_gene_es[509:567,]
...
x <- sample(1:567,size=n,replace=TRUE)
sam_exp=full_data[x[1:508],]
sam_ctl=full_data[x[409:567],]                ## Bootstrap samples
sam_exp_miRNA=full_data_miRNA[x[1:508],]
sam_ctl_miRNA=full_data_miRNA[x[509:567],]
sam_exp_miRNA_gene_es=full_data_miRNA_gene_es[x[1:508],]
sam_ctl_miRNA_gene_es=full_data_miRNA_gene_es[x[509:567],]
